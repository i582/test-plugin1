package org.ton.tact.lang.doc.psi

import com.intellij.util.text.CharArrayUtil
import com.intellij.util.text.CharSequenceSubSequence

data class TactDocLine(
    ...
    fun removePrefix(prefix: String): TactDocLine {
    ...
    fun substring(startIndex: Int): TactDocLine {
        require(startIndex <= contentLength)
        return copy(contentStartOffset = contentStartOffset + startIndex)
    }

    companion object {
        fun splitLines(text: CharSequence): Sequence<TactDocLine> {
            var prev = 0
            return generateSequence {
                if (prev == -1) return@generateSequence null
                val index = text.indexOf(char = '\n', startIndex = prev)
                if (index >= 0) {
                    val line = TactDocLine(text, startOffset = prev, endOffset = index, isLastLine = false)
                    prev = index + 1
                    line
                } else {
                    val line = TactDocLine(text, startOffset = prev, endOffset = text.length, isLastLine = true)
                    prev = -1
                    line
                }
            }
