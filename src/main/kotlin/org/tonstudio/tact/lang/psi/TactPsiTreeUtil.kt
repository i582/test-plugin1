...
object TactPsiTreeUtil {
    ...
    private fun getElementRange(file: TactFile, startOffset: Int, endOffset: Int): Couple<PsiElement?>? {
    ...
    private fun getTopmostElementRange(elementRange: Couple<PsiElement?>): Couple<PsiElement?>? {
        ...
        if (range.equalsToRange(start, end) ||
            ...
            (children.isEmpty() || children[0].textRange.startOffset > start)
            || range.endOffset == end &&
            (children.isEmpty() || children[children.size - 1].textRange.endOffset < end)
        ) {
